...
class FollowSearchGlobalProperty(RetryingThingRunner):
    ...
    def __init__(self, config=None):
        ...
        """
        super(FollowSearchGlobalProperty, self).__init__(config=config)
        self.__thing = None
    ...
    def __find_and_bind(self, predicate, point_type):
        """Search the global registry for the predicate(s) you want
        """
        thing_list = self.client.search_property(predicate)
...
def main():
    FollowSearchGlobalProperty(config="agent2.ini").run()
